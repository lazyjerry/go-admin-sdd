...
// 本檔案實作 migrate 子命令，用於：
// - 初始化資料庫連線與執行資料遷移（migrate）
// - 或者根據模板產生 migration 檔案（generate 選項）
//
// 主要流程：
// 1. run(): 根據 flag 決定執行初始化或產生 migration 檔案
// 2. initDB(): 設定資料庫連線並執行 migrateModel()
// 3. migrateModel(): 對資料庫進行 AutoMigrate 並執行 migration 套件的遷移流程
// 4. genFile(): 產生 migration 檔案（以 timestamp 命名）
//
