...
// SysUser 代表後台系統使用者表 `sys_user` 的資料結構。
// 欄位說明：
// - UserId: 使用者編號（主鍵）
// - Username: 帳號
// - Password: 密碼（保存為 bcrypt hash）
// - NickName: 顯示名稱
// - Phone/Email: 聯絡資訊
// - RoleId/DeptId/PostId: 角色/部門/崗位關聯
// - Salt: 可選的加鹽欄位（此專案使用 bcrypt，Salt 欄位可作額外處理）
// - Status: 帳號狀態
//
// 方法說明：
// - Encrypt(): 使用 bcrypt 將明文密碼加密為 hash
// - BeforeCreate(): GORM 的 hook，在建立前自動呼叫 Encrypt()
...
// BeforeCreate GORM hook，在建立使用者前自動加密密碼
